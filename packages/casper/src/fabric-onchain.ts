...
// scoped session key (Casper native weighted associated keys + action thresholds).
...
/** Load Casper + demo-mode config from environment (.env). */
export function fabricConfig(): FabricChainConfig {
  return loadChainConfig();
}

/** Read-only chain status (demo vs live testnet). */
...
/**
 * Pay an x402 quote on Casper — transfer CSPR to the quoted recipient.
...
export async function payThroughSession(args: {
  ...
  if (!st.demoMode && !st.configured) {
    throw new Error(
      "Casper chain not configured — set CHAIN_WORKER_SECRET_KEY and CASPER_PUBLIC_KEY, or FABRIC_DEMO_CHAIN=true",
    );
  }

  args.onStep?.(st.demoMode ? "settling on demo chain" : "submitting casper-client transfer");
