...
import { featuresBentoConfig } from "@/lib/config";
...
function ProtocolStack(): ReactNode {
  return (
    <div className="flex items-center">
      {PROTOCOLS.map((label) => (
        <div
          key={label}
          className="size-12 rounded-full border-2 border-white/25 bg-accent/20 text-card-foreground flex items-center justify-center text-xs font-semibold -ml-4 first:ml-0"
        >
          {label}
        </div>
      ))}
  ...
  return (
    <div className="flex items-center justify-between bg-background rounded-xl p-3">
      <div className="flex items-center gap-2">
        <span className="rounded bg-accent/30 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-neutral-900">
          {tag}
        </span>
  ...
  return (
    ...
    >
      <div className="relative z-10 text-center mb-6 transition-transform duration-500 ease-out group-hover:scale-105">
        ...
        <p className="text-neutral-700 text-sm">{bento.publish.description}</p>
      </div>
        ...
        <PhoneMockup variant="full">
          <div className="absolute inset-0 bg-phone-screen pt-14 px-5">
            <h4 className="text-3xl font-medium text-neutral-900 leading-none tracking-tight mt-4">
              {bento.publish.phoneTitle}
            </h4>
            <h4 className="text-3xl font-medium text-neutral-900 leading-none tracking-tight mb-4">
              {bento.publish.phoneSubtitle}
            </h4>
            <p className="text-sm text-neutral-500 leading-snug mb-8">{bento.publish.phoneBody}</p>
...
function ProjectCardContent(): ReactNode {
  const parts = bento.publish.projectMeta.split(" · ");
  ...
  return (
    <>
      ...
      <div className="relative z-10 flex items-start justify-between gap-3 h-full">
        <div>
          <p className="text-base font-semibold text-neutral-900">{bento.publish.projectName}</p>
          <p className="text-base font-semibold text-neutral-900">published</p>
        </div>
  ...
  return (
    ...
    >
      <div className="relative z-10 max-w-48 transition-transform duration-500 ease-out group-hover:scale-105">
        ...
        <p className="text-card-foreground-muted text-sm">{bento.invoke.description}</p>
      </div>
      ...
      <div className="relative md:absolute mt-8 md:mt-0 md:right-12 md:top-1/2 md:-translate-y-1/2 flex items-center justify-center transition-transform duration-500 ease-out group-hover:scale-105 self-center md:self-auto">
        ...
        <PhoneMockup variant="compact">
          <div className="absolute inset-0 bg-phone-screen pt-9 px-3">
            <div className="bg-white rounded-full px-2 py-1.5 mb-3 flex items-center gap-1.5 border border-neutral-200">
              <span className="text-neutral-400 text-xs">POST /s/hello-weather</span>
            </div>
            <p className="text-xs text-neutral-500 mb-0.5">{bento.invoke.activeLabel}</p>
            <p className="text-xl font-medium text-neutral-900 mb-3">{bento.invoke.activeCount}</p>

            <div className="flex gap-1.5 mb-4">
              <span className="bg-accent text-black text-xs px-2.5 py-1 rounded-full">402</span>
              <span className="text-neutral-400 text-xs px-2 py-1">pay</span>
              <span className="text-neutral-400 text-xs px-2 py-1">run</span>
            </div>
          </div>
        </PhoneMockup>
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 bg-neutral-900 rounded-2xl px-5 py-3 shadow-xl z-20 whitespace-nowrap">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-neutral-400 text-xs">{bento.invoke.statusLabel}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-lg font-medium text-white">{bento.invoke.statusValue}</span>
            <span className="text-xs font-medium text-accent bg-accent/20 px-2 py-0.5 rounded">
              {bento.invoke.statusBadge}
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-5 text-card-foreground-muted transition-transform duration-500 ease-out group-hover:scale-105">
        ...
        <span className="text-xs font-medium">{bento.ecosystem.rating}</span>
      </div>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      <div className="mb-auto transition-transform duration-500 ease-out group-hover:scale-105">
        ...
        <p className="text-neutral-700 text-sm">{bento.scale.description}</p>
      </div>

      <div className="flex flex-col gap-2 mt-6 transition-transform duration-500 ease-out group-hover:scale-[1.02]">
        {bento.scale.stats.map((stat) => (
          <DeploymentStat key={stat.tag} {...stat} />
        ))}
      </div>
    </motion.div>
  );
