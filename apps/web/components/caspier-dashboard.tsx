...
export function CaspierDashboard({ initialTab }: { initialTab?: SectionKey }): ReactNode {
  ...
  const [explorer, setExplorer] = useState("https://testnet.cspr.live");
    ...
    >
      <aside className="border-b border-white/[0.06] bg-white/[0.015] lg:sticky lg:top-0 lg:h-[100dvh] lg:w-[17rem] lg:shrink-0 lg:border-b-0 lg:border-r">
        <div className="flex h-full flex-col gap-3 p-4">
          ...
          >
            <span className="font-mono">Casper {network}</span>
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-white/[0.06] bg-[#080808]/70 px-6 py-3.5 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="hidden items-center gap-1.5 rounded-full border border-accent/30 bg-accent/10 px-3.5 py-1.5 text-xs font-semibold text-accent sm:inline-flex">
              <Eye className="h-3.5 w-3.5" /> Casper {network}
            </span>
          </div>
        </div>
