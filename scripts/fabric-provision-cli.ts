#!/usr/bin/env bun
// Agent Fabric — provision a scoped session key on Casper (demo records intent;
// production: casper-client add-associated-key with weighted threshold).
//
// bun run agent:provision
import { AuthzService } from "@fabric/authz";
import { chainStatus, fabricConfig } from "@fabric/casper/onchain";
import { mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";

const ROOT = join(import.meta.dir, "..");
const BUILD = join(ROOT, "packages", "casper", "build");
...
if (!config.publicKeyHex) {
  console.error("Set CASPER_PUBLIC_KEY in .env");
  process.exit(1);
}
...
const maxSpend = process.argv[3] ?? "5000000000"; // 5 CSPR in motes
const expiresAt =
...
console.log(
  "\nProduction: use casper-client add-associated-key with low weight + action thresholds.",
);
